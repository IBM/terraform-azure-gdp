# Automated installation of GDP appliances on Azure

## Scope

The modules contained here automate installation of GDP appliances onto Azure.
...
1. Connect to Azure. Plan the installation. You will need the following items from Azure. You will probably have to get some of this information from your Azure administrator. The items you need are documented in the [phase0bastion README file](./examples/phase0bastion/README.md)

You will also need the following access keys.
* A `key.json` file that contains an access key for Azure with access to create new machines. You will probably need to get this from your Azure admin.
* A private/public SSH key pair for accessing the GDP machines. This can be created by you using the regular `ssh-keygen` command, and you will then add it in the Azure Console.
...
* Ability to login to Azure and view the instances and other information.
* A `key.json` file that contains an access key for Azure with access to create new machines.
...
Create a GDP Central Manager on Azure:

```hcl
# modules/central_manager/main.tf (Azure Version)
...
resource "google_compute_instance" "cm" {
  ...
  metadata = {
    ...
    startup-script = <<-EOF
      ...
      echo "Startup completed at $(date)" > /var/log/guardium-Azure-startup.log
    EOF
  }
...
Create a GDP Aggregator on Azure:

```hcl
# modules/aggregator/main.tf (Azure Version)
...
Create a GDP Collector on Azure:

```hcl
# modules/collector/main.tf (Azure Version)
