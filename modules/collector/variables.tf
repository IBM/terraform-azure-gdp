...
# modules/collector/variables.tf
...
variable "phase" {
  type        = string
  default     = "3"
  description = "Deployment phase: '3' = VM only, '3-with-basic-config' = VM + basic config"
  
  validation {
    condition     = contains(["3", "3-with-basic-config"], var.phase)
    error_message = "Phase must be '3' or '3-with-basic-config'."
  }
}
