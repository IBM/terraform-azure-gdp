...
# modules/aggregator/variables.tf
...
variable "phase" {
  type        = string
  default     = "2"
  description = "Deployment phase: '2' = VM only, '2-with-basic-config' = VM + basic config"
  
  validation {
    condition     = contains(["2", "2-with-basic-config"], var.phase)
    error_message = "Phase must be '2' or '2-with-basic-config'."
  }
}
